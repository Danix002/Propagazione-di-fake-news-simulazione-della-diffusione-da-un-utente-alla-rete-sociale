...
italy_map = icvc.italy_reference_map_with_cities(show_principal_cities_only=True)
...
for reg in regions_counts:
    ...
    if (not specific_region.empty):
        ...
        specific_region.plot(ax=italy_map, color=color_region, edgecolor=grigio_scuro, linewidth=1)
    else:
